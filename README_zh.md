...
[![规则数量](https://img.shields.io/badge/rules-129-brightgreen)](docs/RULES_zh.md)
...
V1.0.3 聚焦发布成熟度和功能缺口修复：新增项目自身 CI、coverage、mypy、nox、CHANGELOG 和 CONTRIBUTING；移除 PDF 报告此前仅展示前 80 条发现的限制；新增文件系统风险规则；新增安全的 `agent-scan fix` 预览命令，默认只展示补丁式修复建议，不直接修改用户文件。
...
|---|---|---|
| Terminal | 本地快速查看 | 终端 |
| JSON | 自动化和自定义流水线 | 标准输出；使用 `--output` 时写入 `output/machine/report.json` |
| SARIF | GitHub Code Scanning 和安全平台 | `output/machine/agent-scan.sarif` |
| Markdown | PR 评论和人工报告 | `output/<项目名>/<时间戳>/en/<项目名>_<时间戳>_en.md`、`output/<项目名>/<时间戳>/zh/<项目名>_<时间戳>_zh.md` |
| Excel | 审计分拣和表格复核 | `output/<项目名>/<时间戳>/en/<项目名>_<时间戳>_en.xlsx`、`output/<项目名>/<时间戳>/zh/<项目名>_<时间戳>_zh.xlsx` |
| PDF | 可分享的审计风格报告 | `output/<项目名>/<时间戳>/en/<项目名>_<时间戳>_en.pdf`、`output/<项目名>/<时间戳>/zh/<项目名>_<时间戳>_zh.pdf` |
...
- 文件系统风险，例如项目中包含本地凭据文件、敏感主机目录、过宽 `chmod` 权限命令等。
...
| 文件系统检测 | 检测敏感本地文件、主机凭据目录和过宽 `chmod` 权限模式 |
...
| 工程质量配置 | 提供项目自身 CI、coverage、mypy 和 nox 配置 |
...
其中 `en/` 是英文人工阅读报告，`zh/` 是中文人工阅读报告，`machine/` 用于 SARIF、JSON 等机器读取报告。默认文件名格式为 `项目名_时间戳_语言.格式`，同一个项目多次生成报告时会创建不同的时间戳目录，避免互相覆盖。
