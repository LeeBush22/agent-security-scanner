...
V1.0.2 改进了交互式 CLI 在长时间扫描和报告生成时的体验。选项 1、2、3、4 会显示明确的扫描中、生成中和阶段提示；生成报告时会尽量复用上一次匹配的扫描结果；默认报告输出会按照项目名和时间戳分组。人工报告文件名使用 `项目名_时间戳_语言.格式`，例如 `my-project_20260621_153000_zh.pdf`。
